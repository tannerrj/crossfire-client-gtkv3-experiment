...
/* GTK3: use GdkRGBA instead of GTK2 GdkColor for stat bar colors. */
...
void stats_get_styles(void)
{
    ...
    if (!has_init) {
        memset(bar_colors, 0, sizeof(GdkRGBA*) * MAX_STAT_BARS * NUM_STYLES);
        has_init = 1;
    }
 ...
 */
/**
 * Update a single stat bar's fraction and label.
 *
 * GTK3 port notes:
 *   - GdkColor replaced by GdkRGBA (bar_colors[] is now GdkRGBA*).
 *   - GdkRGBA channels are gdouble in [0,1]; old GdkColor channels were
 *     guint16 in [0,65535], so arithmetic in the gradient blend is unchanged.
 *   - gtk_widget_modify_base() and gtk_widget_modify_bg() removed in GTK3.
 *     Replaced with gtk_widget_override_background_color() which accepts
 *     GdkRGBA* (deprecated in GTK 3.16 but available for GTK3 compatibility).
 *     Long-term: apply color via a CSS provider per stat bar widget.
 *
 * @param stat_no       Index into stat_bar[] for the bar to update.
 * @param max_stat      Normal maximum value of the stat.
 * @param current_stat  Current value of the stat.
 * @param statbar_max   Denominator used for computing the bar fraction.
 * @param statbar_stat  Numerator used for computing the bar fraction.
 * @param can_alert     Unused; kept for API compatibility.
 */
void update_stat(int stat_no, gint64 max_stat, gint64 current_stat,
                 gint64 statbar_max, gint64 statbar_stat, int can_alert)
{
    float bar;
    /* GTK3: use GdkRGBA for blended stat bar colors. */
    ...
    if (use_config[CONFIG_GRAD_COLOR]) {
        /* Blend the bar color continuously from low to normal to super. */
        ...
        if (lcolor && hcolor) {
            /* Linear interpolation between lcolor and hcolor. */
            ncolor = *lcolor;
    ...
    /*
     * GTK3: gtk_widget_modify_base() and gtk_widget_modify_bg() are removed.
     * gtk_widget_override_background_color() accepts GdkRGBA* and is
     * available in GTK3 (deprecated in 3.16; use a CSS provider for 3.20+).
     */
    if (set_color) {
        gtk_widget_override_background_color(stat_bar[stat_no],
                                             GTK_STATE_FLAG_NORMAL, set_color);
    } else {
        /* NULL resets the color to the theme default. */
        gtk_widget_override_background_color(stat_bar[stat_no],
    ...
    /* Display experience using SI prefixes; everything else as plain numbers. */
