...
/** Global RGBA color table used by the magic map and text coloring. */
 ...
 * Should be called after config_load() so defaults are in place first.
 */
static void parse_args(int argc, char *argv[]) {
    GOptionContext *context = g_option_context_new("- Crossfire GTKv3 Client");
    GError *error = NULL;
...
static void init_ui() {
    ...
    /* GTK3: connect "draw" signal instead of deprecated "expose_event". */
    g_signal_connect((gpointer) magic_map, "draw",
                     G_CALLBACK(on_drawingarea_magic_map_draw), NULL);

    /* Set up colors before doing the other initialization functions.
     * GTK3: use gdk_rgba_parse() + GdkRGBA instead of GTK2 GdkColor. */
...
int main(int argc, char *argv[]) {
    ...
    snprintf(VERSION_INFO, MAX_BUF,
            "GTKv3 Client " FULL_VERSION " (%s)", layout);
    g_free(layout);
...
/**
 * Gets the screen coordinates of a specified widget window.
 *
 * GTK3 note: gdk_window_get_geometry() no longer has a depth parameter.
 *
 * @param win  Widget whose window coordinates are requested.
 * @param x    Parent-relative x coordinate (output).
 * @param y    Parent-relative y coordinate (output).
 * @param wx   Root window x coordinate (output).
 * @param wy   Root window y coordinate (output).
 * @param w    Window width (output).
 * @param h    Window height (output).
 */
void get_window_coord(GtkWidget *win, int *x, int *y, int *wx, int *wy,
        int *w, int *h) {
    /* GTK3: gdk_window_get_geometry() takes 4 args (no depth parameter). */
