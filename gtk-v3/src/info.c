...
/*
 * GTK3 note: set_text_tag_from_style() has been removed.
 * In GTK2 it extracted color/font information from a GtkStyle object obtained
 * via gtk_rc_get_style_by_paths().  Both GtkStyle and gtk_rc_get_style_by_paths()
 * are absent from GTK3.  Text tag styling is now handled by applying named CSS
 * classes to widgets, or by setting tag attributes directly when colors are
 * known at compile time.  The add_style_to_textbuffer() function below now
 * clears any previously set tag attributes (graceful degradation).
 */
...
/**
 * Reset style-derived tag attributes on the given message pane.
 *
 * GTK3 port: the GTK2 version accepted a GtkStyle* and called
 * gtk_rc_get_style_by_paths() to look up color/font overrides from the RC
 * file system.  In GTK3 neither GtkStyle nor gtk_rc_get_style_by_paths()
 * exist.  This function now simply clears any previously applied color and
 * font tags so the text falls back to the widget's CSS-defined defaults.
 * Proper GTK3 theming can be achieved by naming text tags and adding CSS
 * selectors for "textview text > .tag-name" in the client theme CSS file.
 *
 * @param pane Message panel whose style tags should be reset.
 */
 ...
 */
/**
 * Initialize or refresh text tag styles for all message panes.
 *
 * GTK3 port: the GTK2 version used gtk_rc_get_style_by_paths() to look up
 * named styles from the RC file system (e.g. "info_default", "msg_*") and
 * applied color/font differences via set_text_tag_from_style().  In GTK3
 * neither GtkStyle nor gtk_rc_get_style_by_paths() are available.
 *
 * This function now only allocates the tag infrastructure on first call.
 * Color and font overrides must be configured via the CSS theme file.
 * The has_style flag is left at 0 so the caller falls back to orig_color.
 */
void info_get_styles(void)
{
    ...
    if (!has_init) {
        /*
         * Build a 2D array of msg_type_tags indexed by [type][subtype].
         * Determine max_subtype from the msg_type_names table first.
         */
    ...
    /* GTK3: RC-based style lookup is unavailable; clear any stale tags. */
    has_style = 0;

    for (i = 0; i < sizeof(msg_type_names) / sizeof(Msg_Type_Names); i++) {
        int type    = msg_type_names[i].type;
        int subtype = msg_type_names[i].subtype;
        for (j = 0; j < NUM_TEXT_VIEWS; j++) {
            if (info_pane[j].msg_type_tags[type][subtype]) {
                gtk_text_tag_table_remove(
                    gtk_text_buffer_get_tag_table(info_pane[j].textbuffer),
    ...
    for (j = 0; j < NUM_TEXT_VIEWS; j++) {
        add_style_to_textbuffer(&info_pane[j]);
    }
}
