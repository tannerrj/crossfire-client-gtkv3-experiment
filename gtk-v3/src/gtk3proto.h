...
/**
 * @file gtk3proto.h
 * Function prototypes for the GTK v3 Crossfire client.
 * Replaces gtk2proto.h: updated to remove GtkStyle and use GdkRGBA/GTK3 APIs.
 */

/* account.c -- Login account management and character selection UI. */
...
/* image.c -- Image cache management and pixmap creation. */
...
/* info.c -- Text message pane drawing; uses GtkStyleContext (GTK3). */
extern void add_tags_to_textbuffer(Info_Pane *pane, GtkTextBuffer *textbuf);
extern void add_style_to_textbuffer(Info_Pane *pane);
...
/* inventory.c -- Inventory and look-window drawing; uses GdkRGBA (GTK3). */
...
/* keys.c -- Keyboard binding and command input handling. */
...
/* main.c -- Client startup, event loop, and top-level window management. */
...
/* map.c -- Map drawing area initialization and rendering. */
...
/* magicmap.c -- Magic map drawing; uses GTK3 draw signal and GdkRGBA. */
extern void draw_magic_map(void);
extern void magic_map_flash_pos(void);
extern gboolean on_drawingarea_magic_map_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data);

/* menubar.c -- Menu bar initialization. */
extern void init_menu_items();

/* metaserver.c -- Metaserver browser UI. */
...
/* pickup.c -- Pickup menu callbacks. */
...
/* png.c -- PNG decoding and RGBA-to-GdkPixbuf/cairo-surface conversion. */
...
/* skills.c -- Skill list display and skill activation. */
...
/* sound.c -- Sound effect and music playback via SDL_mixer. */
...
/* spells.c -- Spell list display; uses GdkRGBA for row coloring (GTK3). */
...
/* stats.c -- Character stat bars and attribute display; uses GdkRGBA (GTK3). */
