...
void on_spell_window_size_allocate(GtkWidget *widget, gpointer user_data)
{
    ...
    /* GTK3: direct allocation field access removed; use accessor instead. */
...
void update_spell_information(void)
{
    ...
    GtkTreeIter iter;
    char buf[MAX_BUF];
    /* GTK3 port: GtkStyle/GdkColor replaced by GdkRGBA; styles are all NULL. */
     ...
     */
    /* GTK3 port: spell_styles are all NULL; label/eventbox colors fall back
     * to CSS defaults.  gtk_widget_modify_fg/bg/font are removed in GTK3. */
