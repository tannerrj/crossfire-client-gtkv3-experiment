 ...
 *
 * GTK3 port notes:
 *   - "expose_event" replaced by the GTK3 "draw" signal; the draw callback
 *     receives a cairo_t* and widget allocation rather than a GdkEventExpose.
 *   - gdk_cairo_create() replaced by cairo_t* supplied by the draw signal.
 *   - map_expose_event() renamed to map_draw_callback() with updated signature.
 */
...
/* GTK3: persistent off-screen surface; composited inside the draw callback. */
static cairo_surface_t *map_surface = NULL;
static float map_surface_scale = 1.0f;

/* Forward declarations for event callbacks. */
static gboolean map_button_event(GtkWidget *widget,
        GdkEventButton *event, gpointer user_data);
/** GTK3 draw callback (replaces GTK2 expose_event). */
static gboolean map_draw_callback(GtkWidget *widget,
...
void map_init(GtkWidget *window_root) {
    ...
    /* GTK3: use "draw" signal instead of deprecated "expose_event". */
    g_signal_connect(map_drawing_area, "draw",
            G_CALLBACK(map_draw_callback), NULL);
...
static void gtk_map_redraw() {
    ...
    /* GTK3: store the completed surface and schedule a draw instead of
     * calling gdk_cairo_create() (deprecated in GTK 3.22, removed in GTK4).
     * The draw callback composites map_surface onto the widget's cr. */
    if (map_surface) {
        cairo_surface_destroy(map_surface);
    }
    map_surface = cst;
    map_surface_scale = scale;
...
/**
 * GTK3 "draw" signal callback for the map drawing area.
 *
 * Replaces the GTK2 "expose_event" callback.  The cairo_t* is provided by
 * GTK and must NOT be destroyed here.  Composites the pre-rendered
 * map_surface (built in gtk_map_redraw()) onto the widget's cairo context.
 * If map_surface is NULL (first frame before any redraw), the area is
 * filled black.
 *
 * @param widget    The map GtkDrawingArea.
 * @param cr        Cairo context for this draw cycle (do not destroy).
 * @param user_data Unused.
 * @return FALSE to allow default GTK processing.
 */
static gboolean map_draw_callback(GtkWidget *widget, cairo_t *cr,
        gpointer user_data) {
    if (map_surface) {
        if (map_surface_scale != 1.0f) {
            cairo_scale(cr, map_surface_scale, map_surface_scale);
        }
        cairo_set_source_surface(cr, map_surface, 0, 0);
        if ((int)(map_surface_scale * 100) % 100 == 0) {
            cairo_pattern_set_filter(cairo_get_source(cr), CAIRO_FILTER_NEAREST);
        }
        cairo_paint(cr);
    } else {
