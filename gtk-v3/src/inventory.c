 ...
 */
/**
 * Adds an item row to the treestore.
 *
 * GTK3 port: foreground/background are now GdkRGBA* (GDK_TYPE_RGBA columns).
 * GtkStyle-based row_style lookup is replaced by get_row_color().
 *
 * @param it     Item to add.
 * @param store  Target GtkTreeStore.
 * @param new    Returns the iterator for the inserted row.
 * @param parent Parent iterator, or NULL for a top-level row.
 * @param color  Non-zero to apply color overrides; 0 for plain display.
 */
static void add_object_to_store(item *it, GtkTreeStore *store,
        GtkTreeIter *new, GtkTreeIter *parent, int color) {
    char buf[256], buf1[256];
    const GdkRGBA *foreground = NULL, *background = NULL;
...
/**
 * Renders an item icon into a drawing area during a GTK3 draw callback.
 *
 * GTK3 port: replaced gdk_cairo_create(GdkWindow*) + gdk_window_clear() with
 * direct use of the cairo_t* supplied by the "draw" signal.  This function
 * must only be called from within a "draw" signal handler.
 *
 * @param cr    Cairo context provided by the GTK3 draw signal.
 * @param image GdkPixbuf to paint; must not be NULL.
 */
static void draw_inv_table_icon(cairo_t *cr, const void *image) {
    gdk_cairo_set_source_pixbuf(cr, (GdkPixbuf *) image, 0, 0);
    cairo_paint(cr);
}

/**
 * GTK3 "draw" signal handler for inventory table drawing areas.
 *
 * GTK3 port: replaces drawingarea_inventory_table_expose_event().  The
 * GdkEventExpose* parameter is gone; GTK3 provides cairo_t* directly.
 *
 * @param widget    The drawing area widget being drawn.
 * @param cr        Cairo context for the current draw operation.
 * @param user_data The item* associated with this drawing area cell.
 * @return TRUE to stop further signal emission.
 */
static gboolean drawingarea_inventory_table_draw(GtkWidget *widget,
        cairo_t *cr, gpointer user_data) {
    if (cpl.ob->inv_updated != 0) {
        /* Delay drawing until inventory is fully updated to avoid
         * use-after-free when items are removed mid-update. */
        return TRUE;
    }

    /*
     * Can get cases when switching tabs that we get a draw event before the
     * list is updated - if so, don't draw stuff we don't have faces for.
     */
    item *tmp = (item *) user_data;
    if (tmp->face) {
        draw_inv_table_icon(cr, pixmaps[tmp->face]->icon_image);
    }
        ...
        if (animate) {
            ...
            if (tmp->animation_id > 0 && tmp->anim_speed) {
                ...
                if (tmp->last_anim >= tmp->anim_speed) {
                    ...
                    /* Queue a redraw; the draw callback paints the new face. */
            ...
            handler = g_signal_handler_find((gpointer) INV_TABLE_AT(x, y, columns),
                    G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                    G_CALLBACK(drawingarea_inventory_table_draw),
                    NULL);
            if (handler) {
                g_signal_handler_disconnect((gpointer) INV_TABLE_AT(x, y, columns), handler);
            }
            ...
            /* GTK3: "draw" replaces "expose_event"; cairo_t* is provided by GTK. */
